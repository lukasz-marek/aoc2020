...
//        sqjhc mxmxvkd sbzzf (contains fish)
        // given
        val foods = listOf(
            Food(setOf("mxmxvkd","kfcds","sqjhc","nhms").map { Ingredient(it) }.toSet(), setOf("dairy", "fish").map { ConcreteAllergen(it) }.toSet()),
            Food(setOf("trh","fvjkl","sbzzf","mxmxvkd").map { Ingredient(it) }.toSet(), setOf("dairy").map { ConcreteAllergen(it) }.toSet()),
            Food(setOf("sqjhc","fvjkl").map { Ingredient(it) }.toSet(), setOf("soy").map { ConcreteAllergen(it) }.toSet()),
            Food(setOf("sqjhc","mxmxvkd","sbzzf").map { Ingredient(it) }.toSet(), setOf("fish").map { ConcreteAllergen(it) }.toSet()))
       // when
